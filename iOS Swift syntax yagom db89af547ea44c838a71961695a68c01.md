...
Last Edited Time: October 3, 2021 5:01 AM
Property: Yagom
Type: 언어
...
- 비반환 함수 (Nonreturning Function)
    - 정상적으로 종료되지 않는 함수를 의미한다. 오류를 던지거나, 중대한 시스템 오류를 보고하는 등의 목적으로 사용한다. 오류를 보고한 이후에 프로세스를 종료한다.
...
- 특정 타입 내부에 또 다른 타입을 선언한 것을 중첩 타입 (Nested Type)이라고 한다.
    
    1) 특정 타입 내부에서만 사용하거나, 2) 특정 타입 외부와의 혼선을 피하고 싶을 때 사용한다.
    
    예를 들어, 구조체 Sports 및 구조체 ESports가 각각 내부에 중첩 타입으로 동일한 이름의 열거형 GameInfo를 선언했더라도, 각 열거형의 case는 다르게 구성할 수 있다.
    

## 26-2. 패턴

블로그 글 참고 (L/G Functions에 패턴 매칭에 대해 설명함) [https://applecider2020.tistory.com/18](https://applecider2020.tistory.com/18)

- 패턴이란?
    
    패턴은 "단독 또는 복합 값의 구조를 나타내는 것"이고, 패턴 매칭은 "코드에서 어떤 패턴의 형태를 찾아내는 행위"이다.
    
    패턴은 크게 두 가지로 구분한다.
    
    - 값을 해체/추출하거나 무시하는 패턴 : 와일드카드 패턴, 식별자 패턴, 값 바인딩 패턴, 튜플 패턴
    - 패턴 매칭을 위한 패턴 : 열거형 케이스 패턴, 옵셔널 패턴, 표현 패턴, 타입캐스팅 패턴
- 열거형 케이스 패턴
    
    특정 값을 열거형의 case에 매치시킨다.
    
    열거형 케이스 패턴은 1) switch의 case 레이블, 2) if, while, guard, for-in문의 case 조건에서 볼 수 있다.
    
    연관값이 있는 열거형 case와 매치하려면, 열거형의 case 패턴에 반드시 튜플 패턴이 함께 해야 한다.
    
    ```swift
    let someValue: Int = 30
    
    if case 0...100 = someValue { // if case 사용?????? if문의 case 조건
    ...
    if case .pasta(let taste) = dinner { // 1) dinner의 값이 case paste가 맞는지 확인, 2) 맞으면 연관값을 상수 taste에 할당
        print("\(taste) 파스타")
    } // 크림 파스타
    ...
    func whatIsThis(dish: MainDish) {
        guard case .pizza(let dough, let topping) = dinner else { // guard case도 가능
    ...
    dishes.append(dinner)
    while case .chicken(let sauced) = dinner { // while case도 가능
...
- 옵셔널 패턴
    ...
    1) 옵셔널 또는 2) 암시적 추출 옵셔널 열거형에 감싸져 있는 값을 매치할 때 사용한다.
    
    열거형 케이스 패턴과 동일하게, 1) switch의 case 레이블, 2) if, while, guard, for-in문의 case 조건에서 볼 수 있다.
    ...
    - [ ]  value는 if-let 사용하듯이 자동으로 언래핑 된건가?
- 타입캐스팅 패턴
    - is 패턴 : 우측의 타입, 또는 우측의 타입의 자식클래스 타입이면 매치한다.
    - as 패턴 : 우측의 타입, 또는 우측의 타입의 자식클래스 타입이면 매치한다. 매치된 타입은 캐스팅된다.
    
    ```swift
    let someValue: Any = 100
    
    switch someValue {
    case is String: print("It's String!")   // 타입 확인 (캐스팅은 안함) - Any는 String이 아니므로 매치 안됨
    case let value as Int: print(value + 1) // 타입 확인 및 캐스팅 (값 바인딩 패턴과 결합된 형태임) - Int는 Any의 자식 타입이므로?????? 매치됨
    default: print("String도, Int도 아니다")
    } // 101
    ```
    
    - [ ]  Int는 Any의 자식 타입이므로?????? 매치됨
- 표현 패턴 (Expression Pattern)
    - 표현식 (수식)의 값을 평가한 결과를 이용하는 것이다. switch문의 case 레이블에서만 사용 가능하다.
    - 패턴 연산자 ~=의 연산 결과가 true이면 매치한다. (~= 연산자는 같은 타입의 두 값을 비교할 때 == 연산자를 사용한다.) 표현 패턴은 정수값, 정수의 범위를 나타내는 Range 객체와 매치할 수 있다.????
        - [ ]  
    - 매우 유용한 패턴이며, 궁극의 패턴 매칭을 만든다. 1) ~= 연산자를 중복정의하거나, 2) 새로 정의하거나, 3) 사용자 정의 타입에 ~= 연산자를 구현하면, 원하는 패턴을 완성할 수 있기 때문이다.
    - 프로토콜 및 제네릭을 사용하면 더 유용하다.
        - [ ]  제네릭 배우고 다시 읽기
    ...
    default: print("0과 5 사이가 아님")
    } // 0과 5 사이
    
    var point: (Int, Int) = (0, 0)
    switch point {
    case (0, 0): print("원점")
    case (-2...2, -2...2): print("\(point.0), \(point.1)은 원점과 가깝다") // 튜플 요소에 접근 (.0 & .1)
    default: print("원점과 가깝지 않다")
    } // 원점
    
    // ~= 연산자 정의
    func ~= (pattern: String, value: Int) -> Bool { // String-Int 매칭이 가능해짐
        return pattern == "\(value)"
    }
    
    switch point {
    case ("0", "0"): print("원점") // 원래 (0, 0)에만 매칭이 가능한데, ~= 연산자를 정의해서 ("0", "0")에도 매칭이 가능해짐
    default: print("원점이 아님")
    } // 원점
    ...
    let kevin: Person = Person(name: "kevin", age: 100)
    
    func ~= (pattern: String, value: Person) -> Bool { // String-Person 매칭이 가능해짐
        return pattern == value.name
    }
    func ~= (pattern: Person, value: Person) -> Bool { // Person-Person 매칭이 가능해짐
        return pattern.name == value.name && pattern.age == value.age
    }
    
    switch kevin {
    case Person(name: "kevin", age: 100): print("동일한 사람이다!") // Person-Person 매칭
    case "james": print("Hello, james") // Person-String 매칭
    default: print("누구세요")
    } // 동일한 사람이다!
    ```
